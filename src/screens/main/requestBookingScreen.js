...
const RequestBookingScreen = ({ route, navigation }) => {
  ...
  const validateForm = () => {
    ...
    if (!selectedPackage?.start_time) {
      ...
      // If package has start_time, use it instead of user input
      setFormData(prev => ({
        ...prev,
        pickup_time: selectedPackage.start_time
      }));
    }
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const bookingData = {
        ...
        pickup_address: formData.pickup_address || selectedPackage?.pickup_location || '',
      };
      ...
      // Handle driver availability warnings - but show success if booking was created
        ...
        if (errorMessage.includes('Your booking request has been submitted')) {
          ...
          // Show success message since the booking system should allow all tourist bookings
          showSuccess(`Your booking request has been submitted! We'll notify the driver and get back to you soon. If the driver doesn't accept within 6 hours, we'll suggest alternative dates or drivers.`, {
            title: 'Booking Submitted Successfully',
          });
          // Show success modal since booking should be created as pending
          setTimeout(() => {
            setBookingReference('PENDING');
            setShowSuccessModal(true);
          }, 1500);
        ...
        <View style={styles.formOuterPad}>
          ...
          <View style={styles.formCard}>
            ...
            <View style={styles.readonlyPackageRow}>
              ...
              <Text style={styles.packagePrice}>₱{selectedPackage?.price || 0}</Text>
            </View>
            ...
            <View style={styles.row}>
              ...
              <View style={[styles.inputGroup, { flex: 1, marginLeft: 8 }]}>
                ...
                {(selectedPackage?.start_time && selectedPackage.start_time.trim() !== '' && selectedPackage.start_time !== 'null' && selectedPackage.start_time !== 'undefined') ? (
                  <View style={styles.readonlyField}>
                    <Ionicons name="time-outline" size={16} color={MAROON} style={{ marginRight: 8 }} />
                    <Text style={styles.readonlyText}>
                      {selectedPackage.start_time} (Fixed)
                    </Text>
                  </View>
                ) : (
                  <TouchableOpacity
...
const styles = StyleSheet.create({
  ...
  pickerText: { fontSize: 14, color: TEXT, flex: 1, marginRight: 8, fontWeight: '600' },
  packagePrice: { fontSize: 16, color: MAROON, fontWeight: '800' },
